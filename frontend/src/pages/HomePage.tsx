...
function ago(time: number, nowMs: number) {
  const ms = nowMs - time;
    ...
    return latestRepos.map((repo) => {
      ...
      return (
        <li className="live-item" key={`${repo.owner}/${repo.repository}`}>
          ...
          <span className="mono">
            {repo.isFork ? "FORK / CONTRIBUTION" : "OWNED PROJECT"} | {repo.language || "Unknown"} | {repo.stars} stars | <time dateTime={repo.updatedAt} title={exactDateFormatter.format(time)}>{ago(time, nowMs)}</time>
          </span>
        </li>
      );
    });
    ...
    return (data?.events ?? []).slice(0, 30).map((event, idx) => {
      ...
      return (
        <li className="live-item" key={`${event.repository}-${event.createdAt}-${idx}`}>
          ...
          <span className="mono"><time dateTime={event.createdAt} title={exactDateFormatter.format(time)}>{ago(time, nowMs)}</time></span>
        </li>
      );
    });
          ...
          {dataUnavailable ? (
            ...
            <>
              <div className="gauges">
                <div className="gauge" role="meter" aria-valuenow={activeThisQuarter} aria-valuemin={0} aria-valuemax={totalProjects} aria-valuetext={`${activeThisQuarter} of ${totalProjects} active this quarter`}>
                ...
                <div className="gauge" role="meter" aria-valuenow={ownedCount} aria-valuemin={0} aria-valuemax={totalProjects} aria-valuetext={`${ownedCount} of ${totalProjects} owned`}>
                ...
                <div className="gauge" role="meter" aria-valuenow={coreCount} aria-valuemin={0} aria-valuemax={ownedCount} aria-valuetext={`${coreCount} of ${ownedCount} core focus`}>
                ...
                <div className="gauge" role="meter" aria-valuenow={topLangCount} aria-valuemin={0} aria-valuemax={totalProjects} aria-valuetext={`${topLangCount} of ${totalProjects} top language`}>
              ...
              {topProjects.map((project, idx) => (
                <li className="row" key={`${project.owner}/${project.repository}`}>
                  ...
                  <span className={statusClass(project.category, project.isFork)}>
                    <Link to={toProjectPath(project.repository)} aria-label={`${statusLabel(project.category, project.isFork)}: View details for ${project.repository}`}>{statusLabel(project.category, project.isFork)}</Link>
                  </span>
                </li>
              ))}
            </ol>
