...
export function HomePage() {
  ...
  const renderedLatestRepos = useMemo(() => {
    ...
    return latestRepos.map((repo) => {
      // ⚡ Bolt: Pre-calculate Date.parse once per iteration to eliminate
      // redundant string parsing overhead for both exactDateFormatter and ago functions.
      const updatedTimeMs = Date.parse(repo.updatedAt);
      return (
        <li className="live-item" key={`${repo.owner}/${repo.repository}`}>
          ...
          <span className="mono">
            {repo.isFork ? "FORK / CONTRIBUTION" : "OWNED PROJECT"} | {repo.language || "Unknown"} | {repo.stars} stars | <time dateTime={repo.updatedAt} title={exactDateFormatter.format(updatedTimeMs)}>{ago(updatedTimeMs, nowMs)}</time>
          </span>
        </li>
      );
    });
  ...
  const renderedEvents = useMemo(() => {
    ...
    return (data?.events ?? []).slice(0, 30).map((event, idx) => {
      ...
      const createdTimeMs = Date.parse(event.createdAt);
      return (
        <li className="live-item" key={`${event.repository}-${event.createdAt}-${idx}`}>
          ...
          <span className="mono"><time dateTime={event.createdAt} title={exactDateFormatter.format(createdTimeMs)}>{ago(createdTimeMs, nowMs)}</time></span>
        </li>
      );
    });
