...
export function HomePage() {
  ...
  const renderedLatestRepos = useMemo(() => {
    ...
    return latestRepos.map((repo) => {
      // ⚡ Bolt: Pre-calculate Date.parse() to avoid redundant parsing inside both formatting and calculation functions.
  ...
  const renderedEvents = useMemo(() => {
    ...
    return (data?.events ?? []).slice(0, 30).map((event, idx) => {
      ...
      // ⚡ Bolt: Pre-calculate Date.parse() to avoid redundant parsing in map loops.
        ...
        <article className="panel">
          ...
          <div className="kpis">
            ...
            <div className="kpi"><b>{latest ? (() => {
              ...
              return <time dateTime={latest} title={exactDateFormatter.format(time)}>{ago(time, Date.now())}</time>;
