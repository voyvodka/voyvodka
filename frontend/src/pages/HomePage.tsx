...
// ⚡ Bolt: Accept pre-parsed numeric timestamp to avoid redundant Date allocations inside loop.
...
export function HomePage() {
  ...
  const renderedLatestRepos = useMemo(() => {
    ...
    return latestRepos.map((repo) => {
      const repoTime = Date.parse(repo.updatedAt);
      return (
        <li className="live-item" key={`${repo.owner}/${repo.repository}`}>
          ...
          <span className="mono">
            {repo.isFork ? "FORK / CONTRIBUTION" : "OWNED PROJECT"} | {repo.language || "Unknown"} | {repo.stars} stars | <time dateTime={repo.updatedAt} title={exactDateFormatter.format(repoTime)}>{ago(repoTime, nowMs)}</time>
          </span>
        </li>
      );
    });
    ...
    return (data?.events ?? []).slice(0, 30).map((event, idx) => {
      ...
      return (
        <li className="live-item" key={`${event.repository}-${event.createdAt}-${idx}`}>
          ...
          <span className="mono"><time dateTime={event.createdAt} title={exactDateFormatter.format(eventTime)}>{ago(eventTime, nowMs)}</time></span>
        </li>
      );
    });
