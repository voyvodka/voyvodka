...
export function HomePage() {
  ...
  const renderedLatestRepos = useMemo(() => {
    ...
    return latestRepos.map((repo) => {
      ...
      const repoUpdatedAtTime = Date.parse(repo.updatedAt);
      return (
        <li className="live-item" key={`${repo.owner}/${repo.repository}`}>
          ...
          <span className="mono">
            {repo.isFork ? "FORK / CONTRIBUTION" : "OWNED PROJECT"} | {repo.language || "Unknown"} | {repo.stars} stars | <time dateTime={repo.updatedAt} title={exactDateFormatter.format(repoUpdatedAtTime)}>{ago(repoUpdatedAtTime, nowMs)}</time>
          </span>
        </li>
      );
    });
  ...
  const renderedEvents = useMemo(() => {
    ...
    return (data?.events ?? []).slice(0, 30).map((event, idx) => {
      ...
      // ⚡ Bolt: Extract Date.parse() to a single shared variable within the loop block to eliminate redundant string parsing overhead.
      const eventCreatedAtTime = Date.parse(event.createdAt);
      return (
        <li className="live-item" key={`${event.repository}-${event.createdAt}-${idx}`}>
          ...
          <span className="mono"><time dateTime={event.createdAt} title={exactDateFormatter.format(eventCreatedAtTime)}>{ago(eventCreatedAtTime, nowMs)}</time></span>
        </li>
      );
    });
        ...
        <article className="panel">
          ...
          <div className="kpis">
            ...
            {(() => {
              ...
              return (
                <div className="kpi"><b>{latest ? <time dateTime={latest} title={exactDateFormatter.format(latestTime)}>{ago(latestTime, Date.now())}</time> : "--"}</b><span className="mono">last push age</span></div>
              );
            })()}
          </div>
        </article>
