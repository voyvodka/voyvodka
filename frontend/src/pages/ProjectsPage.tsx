...
export function ProjectsPage() {
  ...
  const renderedProjects = useMemo(() => {
    ...
    return data.projects.map((project, idx) => {
      ...
      // overhead for exactFormatter and githubDate() calculations in loop.
      ...
      return (
        <li className="row row--full" key={`${project.owner}/${project.repository}`}>
          ...
          <div className="mono updated-col"><time dateTime={project.updatedAt} title={exactDateFormatter.format(updatedAtTime)}>{githubDate(updatedAtTime, nowMs, currentYearStart, currentYearEnd)}</time></div>
