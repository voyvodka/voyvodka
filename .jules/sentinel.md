...
## 2024-05-28 - [Path Traversal in URL Path Parameters]
**Vulnerability:** Path traversal causing SSRF. The backend API used `r.PathValue` to capture `owner` and `repo` names directly from the incoming URL, and blindly interpolated them into internal `fmt.Sprintf` calls to construct downstream GitHub API URLs. Although standard `http.ServeMux` normalizes bare `../` paths, it doesn't strip encoded traversals (like `..%2f`) from `PathValue`. An attacker could pass `..%2fuser` to hit `https://api.github.com/user/readme` instead of a repository endpoint.
**Learning:** `http.ServeMux` path variables (`r.PathValue`) should never be trusted as safe, normalized path segments. When a URL path parameter is used to build another path (for a file system or an external API), it opens up path traversal and Server-Side Request Forgery risks.
**Prevention:** Always apply strict allowlist validation to URL path parameters before interpolating them. For instance, validate that GitHub owner and repo parameters consist strictly of alphanumeric characters, hyphens, underscores, and periods, and fall within expected length bounds.
