...
## 2026-09-07 - [Query String Extraction Bug via URL-Encoded Path Length Mismatch]
**Vulnerability:** The trailing slash normalization middleware in Express incorrectly extracted the query string using `req.url.slice(req.path.length)`. URL-encoded characters in `req.url` (e.g., `%20`) are decoded in `req.path` (e.g., ` `), leading to length mismatches between the two. This caused the substring operation to cut at the wrong character, corrupting the extracted query string or potentially exposing query parameters to manipulation.
**Learning:** `req.path` is URL-decoded, while `req.url` is raw. When operations attempt to use the length of a decoded path against the raw URL string, any presence of URL-encoded characters will cause the operation to read incorrect indices.
**Prevention:** When normalizing URLs or extracting query strings in Express middleware, avoid using substring operations based on `req.path.length` against `req.url` (e.g., `req.url.slice(req.path.length)`). Extract query strings using safe methods like `req.url.indexOf('?')` or by parsing the URL with `new URL(req.url, 'http://localhost')`.
## 2026-09-07 - [Update Dependencies to fix GO/NPM Vulnerabilities]
**Vulnerability:** Go 1.25.0 had numerous standard library vulnerabilities found via `govulncheck`. Also the frontend package `qs` had vulnerabilities.
**Learning:** CI checks run Govulncheck on Go code and `pnpm audit` on Node packages. If standard library vulnerabilities exist, upgrading the Go version in `go.mod` is required. If Node packages are vulnerable, `pnpm update` is required.
**Prevention:** Regularly bump the Go version in `go.mod` to the latest patch release (e.g., `1.25.13`) and run `pnpm update <package>` to resolve audit warnings.
