...
## 2026-07-01 - [Unsafe Query Extraction via Path Length Mismatch]
**Vulnerability:** When normalizing trailing slashes and performing redirects in Express, the query string was extracted using `req.url.slice(req.path.length)`. URL-encoded characters in `req.url` (e.g., `%20`) are decoded in `req.path` (e.g., a space). This caused a length mismatch between the two strings, which corrupted the extracted query string or potentially exposed redirect logic to manipulation if the URL contained encoded sequences.
**Learning:** In Express, `req.url` is the raw, unparsed request URL, while `req.path` is decoded. You cannot reliably use the length of a decoded string to slice an encoded string. This leads to subtle substring misalignment and potential security issues during URL rewriting or redirection.
**Prevention:** When extracting query strings in Express middleware, never use substring operations based on `req.path.length` against `req.url`. Instead, directly locate the query string delimiter (e.g., `req.url.indexOf("?")`) or parse the URL explicitly using the WHATWG `URL` constructor.
