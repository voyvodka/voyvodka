...
## 2026-06-02 - Fix SSRF and Path Traversal in Express API Proxy
**Vulnerability:** The SSR frontend proxy at `/api` decoded `req.url` using `decodeURIComponent` and constructed the downstream target URL using this decoded string. This allowed an attacker to send `%2e%2e` (or `%2f..%2f`) which became `..` after decoding, causing the `URL` constructor to resolve the path upwards and bypass routing restrictions, granting access to internal endpoints like `/api/internal/refresh`.
**Learning:** Decoding URL components before passing them to the WHATWG `URL` constructor causes the constructor to interpret the decoded dots as directory traversal sequences.
**Prevention:** Always use the raw `req.url` directly with the `URL` constructor to extract `pathname` and `search` properties safely. Do not use a decoded URL to construct downstream request targets in a proxy.
