...
## 2024-06-04 - Semantic Time Wrappers for Accessibility
**Learning:** Abstract time categories (e.g., "2024 Q2") and relative dates lack precision for sighted users and context for screen readers. Interpolating React components (like semantic `<time>` wrappers) directly into template strings causes them to render as `[object Object]`.
**Action:** Always wrap abstract and relative dates in a `<time dateTime={isoString}>` element with a `title` attribute for exact date presentation. Use proper JSX fragments when mixing text and components.
...
## 2025-06-05 - Semantic roles for custom gauges and visual noise reduction
**Learning:** Data visualizations like custom metric gauges (e.g., "10/20" completion stats) built with separate `div`s are announced by screen readers as fragmented, disjointed text ("Active this quarter", "10", "/20"), losing their semantic meaning as a fractional measurement. Furthermore, visual-only column headers above CSS grid lists (like a projects list) are read out of context before the list, creating meaningless noise.
**Action:** Always wrap custom gauges with `role="meter"`, providing `aria-valuenow`, `aria-valuemin`, `aria-valuemax`, and a descriptive `aria-valuetext` (like "10 out of 20"). Apply `aria-hidden="true"` to the internal fragmented text nodes. Additionally, apply `aria-hidden="true"` to visual-only grid column headers that precede a list, allowing the screen reader to focus on the semantic list items directly.
